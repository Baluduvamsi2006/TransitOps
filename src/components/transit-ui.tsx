...
export function MetricCard({ label, value, tone = "neutral" }: MetricCardProps) {
  ...
  return (
    <article className="rounded-[1.4rem] border border-white/8 bg-(--panel) p-5 shadow-[0_18px_40px_rgba(0,0,0,0.18)]">
...
export function PageHeader({ eyebrow, title, description }: PageHeaderProps) {
  return (
    <div className="mb-6 space-y-3">
      ...
      <p className="max-w-3xl text-sm leading-7 text-(--muted-2) sm:text-base">{description}</p>
    </div>
  );
}
